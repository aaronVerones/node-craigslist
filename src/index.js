...
/**
 * Accepts string of HTML and parses that string to find number of pertinent postings.
 *
 ...
 **/
function _getNumberOfPostings(options, markup) {
  let $ = cheerio.load(markup);
  return parseInt($("span.totalcount").first().text());
}
...
function _getPostings(options, markup) {
  ...
  $("div.content")
    .find(".result-row")
    .each((i, element) => {
      let // introducing fix for #11 - Craigslist markup changed
        details = $(element)
          .find(".result-title")
          .attr("href")
          .split(/\//g)
        ...
        // fix for #6 and #24
        detailsUrl = url.parse($(element).find(".result-title").attr("href"));
      ...
      return self.request
        ...
        .then((markup) => {
          ...
          if (options.countOnly) {
            postings = _getNumberOfPostings(requestOptions, markup);
          } else {
            postings = _getPostings(requestOptions, markup);
          }
